...
class Repository:
    def __init__(self,url="sqlite:///./data/bot.db"):
        raw=str(url)
        self.path=Path(raw.removeprefix("sqlite:///"))
    ...
    def log_event(self,kind,payload):
        with self.lock:
            with self._conn() as c:
                c.execute("INSERT INTO events(created_at,kind,payload) VALUES(?,?,?)",
                          (datetime.now(timezone.utc).isoformat(),kind,json.dumps(payload,ensure_ascii=False)))

    def recent(self,limit=50):
        with self.lock:
            with self._conn() as c:
                return c.execute("SELECT kind,payload,created_at FROM events ORDER BY id DESC LIMIT ?",(max(1,min(limit,500)),)).fetchall()
