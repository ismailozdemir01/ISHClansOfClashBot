...
from bot.core.models import RuntimeStatus
...
def bind(runtime):
    ...
    async def status():
        s=runtime.status
        return {
            "state":s.state.value,"connected":s.connected,"running":s.running,
            "searches":s.searches,"attacks":s.attacks,"errors":s.errors,
            "last_error":s.last_error,
            "target":s.target.__dict__ if s.target else None,
            "metadata":s.metadata,
        }
    ...
    async def events(limit:int=50):
        return {"events":await runtime.events.recent(limit)}
